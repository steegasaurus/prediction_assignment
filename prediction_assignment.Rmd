...
set.seed(150)
inTrain <- createDataPartition(trainClean$classe, p = .6)
training <- trainClean[inTrain, ]
temp <- trainClean[-inTrain, ]
set.seed(200)
inValidation <- createDataPartition(temp$classe, p = .5)
...
set.seed(300)
fitADA <- train(classe ~ ., method = 'adaboost', trControl = tc, data = trainPCA)
